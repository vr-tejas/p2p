...
public class PeerClean {
    ...
    private ServerThreadClean serverThread;     // Thread handling incoming connections
    private ClientHandlerClean clientHandler;   // Handler for outgoing connections
    ...
    private void startServer() {
        try {
            serverThread = new ServerThreadClean(myPort);
            serverThread.start();
    ...
    private void createSampleFiles() {
        ...
        FileManagerClean.createSampleFile("hello.txt", 
        ...
        FileManagerClean.createSampleFile("info.txt",
            "=== PEER INFORMATION ===\n" +
            "Peer Port: " + myPort + "\n" +
        ...
        FileManagerClean.createSampleFile("data.txt",
    ...
    private void createInitialSampleFiles() {
        List<String> files = FileManagerClean.getAvailableFiles();
        if (files.isEmpty()) {
    ...
    private void connectToPeer() {
        ...
        try {
            ...
            clientHandler = new ClientHandlerClean(ip, port);
    ...
    private void testPeerConnectivity() {
        ...
        try {
            int port = Integer.parseInt(portStr);
            ClientHandlerClean.testPeerReachability(ip, port);
        } catch (NumberFormatException e) {
    ...
    public static void main(String[] args) {
        ...
        if (args.length != 1) {
            System.out.println("[ERROR] Usage: java PeerClean <port>");
            System.out.println("[HELP] Example: java PeerClean 5000");
            System.out.println("\n[START] Quick Start Guide:");
            System.out.println("1. Terminal 1: java PeerClean 5000");
            System.out.println("2. Terminal 2: java PeerClean 5001");
        ...
        try {
            ...
            // Create and run peer
            PeerClean peer = new PeerClean(port);
            peer.run();
